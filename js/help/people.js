...
orbitScreenHelp({
  ...
  "hr.heads": {
    ...
    after: "Nothing posts when you save a head. Heads are used when a payslip is computed, on a payslip or through Generate payslips on a run, and every amount is rounded to two decimals. Payslips already computed keep their lines: changing a head only affects payslips computed afterwards, so regenerate draft payslips after a change. When a payslip is posted, gross pay is debited to a salary expense account, net pay is credited to the Salaries payable account and deductions to the Payroll deductions account, both chosen in Settings, Companies; employer costs are debited to the same expense account and credited to the deductions account as a provision.",
    links: [
  ...
  "hr.runs": {
    ...
    after: "Post all writes one journal entry per payslip in the journal with the code MISC, dated Period To: the gross pay is debited to a salary expense account, net pay credited to a salaries payable account, and deductions credited to a payroll deductions account. Employer costs, when a structure has them, are debited to the same expense account and credited to that deductions account. Net pay goes to the <b>Salaries payable</b> account chosen in Settings, Companies, the same account a Salary payment in Counter starts on, so paying the salary clears it, and deductions go to the <b>Payroll deductions</b> account chosen in the same place. The expense account is chosen by name: an active expense account whose name contains salary, payroll, personnel, wage or staff (otherwise code 6000, otherwise the first expense account). A payslip that fails to post leaves nothing behind in the journal. Each payslip becomes <b>Confirmed</b>. It becomes <b>Paid</b> when the salary is paid from Counter against it. Posted payslips count in Payroll Consolidation. Payroll data is personal: the payroll screens can be switched off for a role in Settings, Roles &amp; Permissions. A payslip that keeps money back and has no Payroll deductions account set refuses to post and says so; nothing is written.",
    links: [
    ...
    mistakes: [
      ...
      ["This payslip keeps money back, but Settings, Companies has no payroll deductions account set (or that account is archived). Choose it under Accounting accounts, then post again.", "Open Settings, Companies, open this company, choose the account tax and social security are owed on in Payroll deductions, Save, and click Post all again. Nothing was posted."],
  ...
  "hr.slips": {
    ...
    mistakes: [
      ...
      ["This payslip keeps money back, but Settings, Companies has no payroll deductions account set (or that account is archived). Choose it under Accounting accounts, then post again.", "Open Settings, Companies, open this company, choose the account tax and social security are owed on in Payroll deductions, Save, and click Confirm &amp; Post again. Nothing was posted."],
    ...
    tips: [
      "Set Salaries payable and Payroll deductions in Settings, Companies once: payslips credit net pay to the first and tax, social security and employer costs to the second, and a Salary payment in Counter starts on the first, so the amount owed clears when the salary is paid.",
